...
function [storespar cr Cn mean_mass mean_res max_intens colmin colmax]=cluster_build(handles,pfun,col1)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
% Control parameter
...
%           list
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% [storespar cr Cn mean_mass mean_res max_intens colmin colmax]=cluster_build(handles)
%
%Part of Masspec:
...
%colmin       Information about the first sample column in Excel
%colmax       Information about the last sample column in Excel
%
...
fprintf(handles.logid,'norefs read \n');%write to log fike

%into which excel column to write the data
dest_col=data(:,4); % column of output excell sheet ascossiated with a sample
off=min(dest_col)-3; % in internal treatment no need for cols reserved mean values (mean mass/intens/resolution)
dest_col=dest_col-off; %internal reference for columns in local matrix
colmin=min(dest_col);%erst spalte
colmax=max(dest_col);%letzte spalte
data(:,4)=data(:,4)-colmin;
clear dest_col
...
soround=(colmax-colmin+1)*6; %preselection in multiples of samplesize                                                             %pre selection inteval
...
fprintf(handles.logid,'overlap matrix calculated \n');%write to log fike
...
%end
close(hw);
fprintf(handles.logid,'finished overlap matrix \n');%write to log fike
%% build clusters
...
for ci=2:length(I_gaps)%loop over initial clusters
    ...
    if ~mod(ci,5000) || ci== length(I_gaps)
        txt=sprintf('build and check intial cluster: %i/%i',ci,length(I_gaps))
        fprintf(handles.logid,[txt '\n']);%write to log fike
    end
...
fprintf(handles.logid,'building cluster sparse matrix \n');%write to log fike
storespar=sparse(CI,CJ,data(:,intens),cr,colmax-colmin+1,l_data);%cluster matrix which rows will be written inbetween existent rows in assigned file
fprintf(handles.logid,'done \n');%write to log fike
...
cols=col1+2*(col1==4)+(colmin:colmax)-3;
